      ...
      "data": [
        {
          ...
          "smiles": "NCC(=O)O",
          "sideChainFormulaTex": "\\ce{-H}",
          "desc": "最も単純なアミノ酸。不斉炭素を持たない唯一のアミノ酸であり、タンパク質の立体構造に柔軟性を与える。",
          "mnemonic": "Glycine の語源はギリシャ語の『glykys（甘い）』に由来し、ゼラチンから結晶として単離されたときの甘味にちなんで名付けられた。"
        }
        // ... 他のアミノ酸行
      ]
    }
...
```jsonc
{
  "id": "p_abbr_to_name_and_class_with_trivia",
  "label": "略号 → 名前＋分類（小ネタ付き）",
  ...
  "tokens": [
...

    {
      ...
      "answer": {
        ...
        "choiceCount": 4,
        "distractorSource": {
          "scope": "filtered",          // entityFilter 適用後の集合から誤答候補を取る
          "count": 3,
          "avoidSameId": true,
          "avoidSameText": true
        }
...

    {
      ...
      "answer": {
        ...
        "choiceCount": 4,
        "distractorSource": {
          "scope": "all",               // DataSet 全体から誤答候補を取る例
          "count": 3,
          "avoidSameId": true,
          "avoidSameText": true
        }
  ...
  "tips": [
    ...
    },
    {
      ...
      "when": "after_incorrect",
      "tokens": [
        { "type": "text", "value": "残念！ここでしっかり覚えておこう：" },
        { "type": "br" },
        { "type": "key",  "field": "desc" },
        { "type": "br" },
        { "type": "text", "value": "語源メモ：" },
...
* 左側にアミノ酸名（`nameJa`）、右側に分類（`classJa`）を表示し、ユーザーに対応付けを行わせる例です。

---

## C. sentence_fill_choice：一問一答文からの n 択穴埋め（DataSet 側）

まず、`factSentences` 型 DataSet として、一問一答文と Group を定義します。
      ...
      "sentences": [
        {
          ...
          "desc": "Proline の側鎖は主鎖のアミノ基と結合して環構造（ピロリジン環）を形成し、ペプチド鎖の構造を曲げやすくする。",
          "mnemonic": "Proline は『環っぽい（ぷろりん）』と覚える。環構造を作る特別なアミノ酸。"
        }
      ],
      "groups": {
        "bondGroup": {
