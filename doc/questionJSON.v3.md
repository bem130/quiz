...
v3 では、**問題ファイルの選択肢を `modes` ではなくディレクトリ構造で生成**します。
...
* `files` は **Main Quiz File へのパス配列**です（相対パス推奨）。
...
* `dataSets` は **各ファイルの `imports` + `dataSets`** で解決し、ファイル間で混在させないことを推奨します。
...
* `patterns` は問題生成ルールの集合
* v3 では `questionRules` / `modes` は使用しません（存在する場合は警告または無視）
...
* Main Quiz File 読み込み時に、
 ...
 1. `imports` に指定されたファイルを順に読み込み、`dataSets` をマージ
  ...
  3. 完成した `dataSets` と `patterns` を用いて問題生成を行う
...
* `patterns` も書いてはいけません（ロジック定義は Main にのみ）
...
* v3 では主に、`factSentences` 型 DataSet 内の `groups` を参照対象とし、`type: "groups"` DataSet は今後の拡張用として位置付けます。
...
* `choices: string[]`
  ...
  * 選択肢候補一覧（v3 では文字列配列を想定）
* `mode: string`

  * この Group の用途を示すメタ情報
  * v3 では主に `"choice"`（n 択候補）を使用
...
* v3 では `"groups"` を起点とする 2 要素配列を想定します：
...
* **制約：**
  ...
  * `tips` 内では **`type: "hide"` は使用できません。**
    ...
    * 小ネタの中に新たな穴埋めを作ることは v3 ではサポートしません。
...
### 9.2 `answer.mode` 一覧（v3）

#### 記述

v3 で使用する `answer.mode` は次の 4 種類です：
