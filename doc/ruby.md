...
読み方に間違いがないよう注意してください  
rubyはtermの中にも外にも書けます  
(例) `[私/わたし]は[漢字/かんじ][仮名/かな][交/ま]じりの[文/ぶん]を[書/か]く`  

# term記法(日本語)
term記法は`{日本語/英語}`或いは`{日本語}`の記法です  
これは、日本語の下に、英語が小さく表示されます  
つまり、`A{B/b}C`と書くと、`ABC`と位置を揃えて描画され、Bの下に小さくbを表示します  
つまり`{ルビ[付/つ]き[日本語/にほんご]/japanese with ruby}`  
ただし、英語を省略した方の記法は基本的に用いないでください  
全ての用語はterm記法で英語名を併記します  
用語でないものにterm記法は適用しません ruby記法のみを用います  
